...
int main() {
    ...
    for (const auto &imagePath : imageList) {
        ...
        TFImage img;
        auto errorCode =
            tfSdk.preprocessImage(image.data, image.cols, image.rows, ColorCode::bgr, img);
        ...
        for (const auto &landmarks : landmarksVec) {
            ...
            setLabel(image, std::to_string(landmarks.score),
                     cv::Point(landmarks.topLeft.x, landmarks.topLeft.y));
        }
        ...
        auto imageName =
            imagePath.substr(imagePath.find_last_of('/') + 1,
                             imagePath.find_last_of('.') - imagePath.find_last_of('/') - 1);
...
std::vector<std::string> getFilesInDir(const std::string &path) {
    ...
    while (iter != end) {
        ...
        if (ec) {
            std::string errMsg =
                "Error While Accessing : " + iter->path().string() + " :: " + ec.message() + '\n';
