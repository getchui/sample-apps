...
//    auto retcode = tfSdk.createDatabaseConnection("host=localhost port=5432 dbname=my_database user=postgres password=admin");
    ...
    // Enroll Faceprint of Brad Pitt in collection 1
    {
        ...
        // TODO: You should run additional quality checks before enrolling. See enroll_in_database.cpp
        bool found;
        Faceprint fp;
        ...
        // Enroll the Faceprint. Since we have multiple collections loaded we must specify the collection name
        std::string UUID;
    ...
    // Now run a search query in collection 1
    {
        ...
        // Query the collection. Since we have multiple collections loaded in memory, we must specify the collection name
        Candidate candidate;
        ...
        if (found) {
            std::cout << "Found candidate in collection: " << collection1 << " with identity: " << candidate.identity << " with match probability: " << candidate.matchProbability << std::endl;
        } else {
            std::cout << "Was not able to find match candidate in collection: " << collection1 << std::endl;
        }
    }
    ...
    // Now run a search query in collection 2
    {
        ...
        if (found) {
            std::cout << "Found candidate in collection: " << collection2 << " with identity: " << candidate.identity << " with match probability: " << candidate.matchProbability << std::endl;
        } else {
            std::cout << "Was not able to find match candidate in collection: " << collection2 << std::endl;
        }
    }
